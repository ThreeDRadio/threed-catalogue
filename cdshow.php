...
<BODY onload="document.forms[0].xwords.focus()">
	...
	if ($bits[0] . "." . $bits[1] . "." . $bits[2] == "192.168.3") {
		$local = 1;
	} else {
		$local = 0;
	}
	;
	if ($user['username'] === 'mmarner') {
		$local = 1;
	}
	...
	for ($i = 0; $i < $num; $i++) {
		...
		if (is_readable("$mp3lopath/$cdnum/$cdnum-$thetracknum.mp3"))
			$mp3lothere[$i] = 1;
		if ($local == 1) {
			if (is_readable("$mp3hipath/$cdnum/$cdnum-$thetracknum.mp3"))
				$mp3hithere[$i] = 1;
		}
	...
	if ($isartist) {
		echo "<TD valign=top><b>Artist</b></TD>";
	}
	if ($istitle) {
		echo "<TD valign=top><b>Title</b></TD>";
	}
	if ($islength) {
		echo "<TD valign=top align=right><b>Length</b></TD>";
	}
	if ($mp3lothere) {
		echo "<TD valign=top align=right><b>LoFi</b></TD>";
	}
	if ($mp3hithere) {
		echo "<TD valign=top align=right><b>HiFi</b></TD>";
	}
	echo "</TR>";
	for ($i = 0; $i < $num; $i++) {
		...
		if ($isartist) {
			if ($trackartist[$i]) {
				echo "<TD valign=top>$trackartist[$i]</TD>";
			} else {
		...
		if ($istitle) {
			if ($tracktitle[$i]) {
				echo "<TD valign=top>$tracktitle[$i]</TD>";
			} else {
				echo "<TD valign=top>&nbsp;</TD>";
			}
		}
		if ($islength) {
			if ($tracklength[$i]) {
				echo "<TD valign=top align=right>$tracklength[$i]</TD>";
			} else {
				echo "<TD valign=top>&nbsp;</TD>";
			}
		}
		if ($mp3lothere) {
			if ($mp3lothere[$i]) {
				echo "<TD valign=top align=right><A HREF=/database/play/$playname[$i]-lo.mp3>play</A></TD>";
			} else {
				echo "<TD valign=top>&nbsp;</TD>";
			}
		}
		if ($mp3hithere) {
			if ($mp3hithere[$i]) {
				echo "<TD valign=top align=right><A HREF=/database/play/$playname[$i]-hi.mp3>play</A></TD>";
			} else {
				echo "<TD valign=top>&nbsp;</TD>";
			}
		}
