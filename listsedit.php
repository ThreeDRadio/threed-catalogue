...
$query = "SELECT * FROM listthing WHERE id = $q$gid$q;";
$result = pg_query($db, $query);
$num = pg_num_rows($result);
if ($num != 1) {
	echo "<p><font color=red><b>List does not exist!</b></font><p>";
	echo "</BODY></HTML>";
	exit;
}
...
if ($xupdate && $gid) {
	$xname = trim($xname);
	$uquery = "UPDATE listthing SET name=$q$xname$q, active=$q$xactive$q, description=$q$xdescription$q WHERE id = $q$gid$q;";
	$result = pg_query($db, $uquery);
	header("Location: http://".$_SERVER['HTTP_HOST'] .dirname($_SERVER['PHP_SELF']) ."/listsmanage.php");
	exit;
}
...
<p>
<form action=listsedit.php method=post>
...
<td><textarea name=xdescription rows=12 cols=50><?php $a=htmlentities($row[description]); echo "$a"; ?></textarea></td>
</tr>
<tr bgcolor="#AAAAFF">
<td><b>Active</b></td>
<td>
