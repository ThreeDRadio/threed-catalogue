...
if ($xdate == "")
	$xdate = date("Y-m-d", strtotime("-7 days"));
if ($xdate2 == "")
	$xdate2 = date("Y-m-d", strtotime("-1 days"));
if ($xperpage < 5)
	$xperpage = 200;
if ($xperpage > 200)
	$xperpage = 200;
...
<BODY onload="document.forms[0].xdate.focus()">
	...
	<form action=session_report_tot.php method=post>
		...
		<table border=0 cellspacing=0 cellpadding=5>
			...
			<tr bgcolor="#CCCCCC">
				<td align=left><b>Enter dates to check&nbsp;(yyyy-mm-dd)&nbsp;From:&nbsp;</b><input type=text name=xdate
						value="<?php echo htmlentities(stripslashes($xdate)) ?>" size=25 maxlength=50>
					<b>&nbsp;To:&nbsp;</b><input type=text name=xdate2
						value="<?php echo htmlentities(stripslashes($xdate2)) ?>" size=25 maxlength=50>
				</td>
				<td></td>
			</tr>
			...
			<tr bgcolor="#CCCCCC">
				<td align=left><b>Date to search:&nbsp;</b>
					<select name=sdate>
						...
						<option value="mod" <?php if ($sdate == "mod") {
							echo " selected";
						} ?>>Date Modified</option>
						<option value="cre" <?php if ($sdate == "cre") {
							echo " selected";
						} ?>>Date Created</option>
						<option value="arr" <?php if ($sdate == "arr") {
							echo " selected";
						} ?>>Date Arrived</option>
					</select>
