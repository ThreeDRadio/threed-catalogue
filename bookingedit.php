...
#echo "xeditdate= :$xeditdate:<br>";
	...
	if ($command == "create") {
		...
		$err = "";
		$tdate = $xdate;
		if (preg_match("|(.*)/(.*)/(.*)|", $xdate, $matches)) {
			$xdate = $matches[2] . "/" . $matches[1] . "/" . $matches[3];
		}
		...
		$startpm = 0;
		$startam = 0;
		if (preg_match("|^([0-9:.]*)([Pp][Mm])$|", $starttime, $matches)) {
			$startpm = 1;
			$starttime = $matches[1];
		}
		if (preg_match("|^([0-9:.]*)([Aa][Mm])$|", $starttime, $matches)) {
			$startam = 1;
			$starttime = $matches[1];
		}
		if (preg_match("|^([0-9]+)[:.]+([0-9][0-9])$|", $starttime, $matches)) {
			$starthour = $matches[1];
			$startmin = $matches[2];
		}
		if (preg_match("|^([0-9])$|", $starttime, $matches)) {
			$starthour = $matches[1];
			$startmin = "0";
		}
		if (preg_match("|^([0-9])([0-9])$|", $starttime, $matches)) {
			$starthour = $matches[1] . $matches[2];
			$startmin = "0";
		}
		if (preg_match("|^([0-9])([0-9])([0-9])$|", $starttime, $matches)) {
			$starthour = $matches[1];
			$startmin = $matches[2] . $matches[3];
		}
		if (preg_match("|^([0-9])([0-9])([0-9])([0-9])$|", $starttime, $matches)) {
			$starthour = $matches[1] . $matches[2];
			$startmin = $matches[3] . $matches[4];
		}
		...
		if ($startmin >= 0 && $startmin <= 59 && $starthour >= 0 && $starthour <= 23) {
			$xstarttime = sprintf("%02.0f", $starthour) . ":" . sprintf("%02.0f", $startmin);
		} else {
			$starterr = 1;
			$err = 1;
		}
		...
		$endpm = 0;
		$endam = 0;
		if (preg_match("|^([0-9:.]*)([Pp][Mm])$|", $endtime, $matches)) {
			$endpm = 1;
			$endtime = $matches[1];
		}
		if (preg_match("|^([0-9:.]*)([Aa][Mm])$|", $endtime, $matches)) {
			$endam = 1;
			$endtime = $matches[1];
		}
		if (preg_match("|^([0-9]+)[:.]+([0-9][0-9])$|", $endtime, $matches)) {
			$endhour = $matches[1];
			$endmin = $matches[2];
		}
		if (preg_match("|^([0-9])$|", $endtime, $matches)) {
			$endhour = $matches[1];
			$endmin = "0";
		}
		if (preg_match("|^([0-9])([0-9])$|", $endtime, $matches)) {
			$endhour = $matches[1] . $matches[2];
			$endmin = "0";
		}
		if (preg_match("|^([0-9])([0-9])([0-9])$|", $endtime, $matches)) {
			$endhour = $matches[1];
			$endmin = $matches[2] . $matches[3];
		}
		if (preg_match("|^([0-9])([0-9])([0-9])([0-9])$|", $endtime, $matches)) {
			$endhour = $matches[1] . $matches[2];
			$endmin = $matches[3] . $matches[4];
		}
		if ($endpm && $endhour < 12) {
			$endhour += 12;
		}
		...
		if ($endmin >= 0 && $endmin <= 59 && (($endhour >= 0 && $endhour <= 23) || ($endhour == 24 && $endmin == 0))) {
			...
			if ($xendtime < $xstarttime && !$starterr) {
				$enderr = 1;
				$err = 1;
			}
		} else {
	...
	<form action=bookingedit.php method=post name=myform>
		...
		<table border=0 cellspacing=0 cellpadding=8>
			...
			<tr bgcolor="#CCCCFF">
				...
				<td colspan=3><input type=text name=xdate value="<?php $a = htmlentities(stripslashes($fdate));
				echo "$a"; ?>" size=30 maxlength=50></td>
			</tr>
			...
			<tr bgcolor="#CCCCFF">
				...
				<td colspan=3><input type=text name=xstarttime value="<?php $a = htmlentities(stripslashes($fstarttime));
				echo "$a"; ?>" size=30 maxlength=50>
			...
			<tr bgcolor="#CCCCFF">
				...
				<td colspan=3><input type=text name=xendtime value="<?php $a = htmlentities(stripslashes($fendtime));
				echo "$a"; ?>" size=30 maxlength=50></td>
			</tr>
			...
			<tr bgcolor="#CCCCFF">
				...
				<td colspan=3><input type=text name=xtext value="<?php $a = htmlentities(stripslashes($ftext));
				echo "$a"; ?>" size=50 maxlength=100></td>
			</tr>
