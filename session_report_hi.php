...
<form action=session_report_hi.php method=post>
...
//	$query ="SELECT cd.id, cd.artist, cd.title, cd.local, cd.format, users.username, cd.arrivaldate, cd.createwhen, cd.modifywhen FROM cd, users ";
	$query=$query.$where;
...
//		echo "<tr><th align=left>ID Number</th><th align=left>Artist</th><th align=left>Album Title</th><th align=left>$head</th><th align=left>ArrivalDate</th><th align=left>CreateDate</th><th align=left>ModifyDate</th><th align=left>Date</th><th align=left>MP3</th><th align=left>Local</th><th align=left>Format</th><th align=center>Action</th></tr>\n";
...
/*
//arrival
			$a = htmlentities($r[cd.arrivaldate]);
			$a=date("Y-m-d", $a);
			echo "<td>";
			if ($a) { echo "$a"; }
			else { echo "&nbsp;"; }
...
//create
/*			$a = htmlentities($r[('1970-01-01 00:00:00 GMT'::timestamp +
...
//modify
			$a = htmlentities($r[('1970-01-01 00:00:00 GMT'::timestamp +
		((cd.modifywhen::bigint)::text)::interval]);
			echo "<td>";
			if ($when!=createdate) {$a=date("Y-m-d", $a);}
			if ($a) { echo "$a"; }
			else { echo "&nbsp;"; }
			echo "</td>\n";
*/			
